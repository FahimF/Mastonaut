...
public class AttachmentsSubcontroller: NSObject {
	@IBOutlet private(set) var attachmentCollectionView: NSCollectionView!
	...
	override public func awakeFromNib() {
		super.awakeFromNib()
		attachmentCollectionView.register(AttachmentItem.self, forItemWithIdentifier: ReuseIdentifiers.attachment)
		attachmentCollectionView.register(ProposedAttachmentItem.self, forItemWithIdentifier: ReuseIdentifiers.proposedAttachment)
		descriptionPopoverViewController.descriptionStringValueDidChangeHandler = {[unowned self] in
			self.updateRemainingDescriptionCountLabel()
		}
		descriptionPopoverViewController.didClickSubmitChangeHandler = {[unowned self] in
	...
	public func collectionViewItem(for upload: Upload) -> AttachmentItem? {
		guard let index = attachments.firstIndex(of: upload), attachmentCollectionView.indexPathsForVisibleItems().contains(IndexPath(item: index, section: 0)) else {
			return nil
		}
...
extension AttachmentsSubcontroller: NSCollectionViewDataSource {
	...
	public func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
		...
		if let attachmentItem = item as? AttachmentItem {
			...
			attachment.loadMetadata {[weak collectionView] metadata in
				DispatchQueue.main.async {
					(collectionView?.item(at: indexPath) as? AttachmentItem)?.set(itemMetadata: metadata)
				}
			}
			attachment.loadThumbnail {[weak collectionView] thumbnail in
				DispatchQueue.main.async {
	...
	private func showDescriptionEditor(for attachment: Upload) {
		...
		descriptionPopoverViewController.set(description: attachment.attachment?.description ?? "", hasError: uploadsWithDescriptionUpdateError.contains(attachment))
		descriptionPopoverViewController.showPopover(relativeTo: attachmentCollectionView.frameForItem(at: index), of: attachmentCollectionView)
		updateRemainingDescriptionCountLabel()
	}

	private func submitDescription() {
		guard let client = client, let index = lastOpenedDescriptionAttachmentIndex, index < attachments.count, index >= 0 else {
			return
		}
		lastOpenedDescriptionAttachmentIndex = nil
...
extension AttachmentsSubcontroller: AttachmentUploaderDelegate {
	...
	public func attachmentUploader(_: AttachmentUploader, produced error: AttachmentUploader.UploadError, for upload: Upload) {
		...
		case let .encodeError(encodeError):
