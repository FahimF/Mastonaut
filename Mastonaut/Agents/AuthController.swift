...
class AuthController {
	...
	func completeAuthorization(from domainName: String, grantCode: String) {
		...
		case let .authorizing(sourceWindow, sheetController, clientApplication, baseDomain, account):
			...
			guard let sheetWindow = sheetController.window,
			      sheetWindow.isSheet, sheetWindow.sheetParent === sourceWindow
			else {
				attemptToAuthorizeFailedTokenGrant(from: domainName, grantCode: grantCode)
				return
			}
	...
	private func updateLocalInfo(for authorizedAccount: AuthorizedAccount, completion: @escaping () -> Void)
	{
		...
		accountsService.details(for: authorizedAccount) {
			...
			guard case let .success(details) = $0, let self = self else {
				completion()
				return
			}
	...
	private func updateBlocksAndMutes(for _: Account,
	                                  authorizedAccount: AuthorizedAccount,
	                                  ...
	                                  completion: @escaping () -> Void)
	{
		...
		service.loadBlockedAccounts {
			...
			DispatchQueue.main.async {
				if case let .success(allBlockedAccounts) = result {
		...
		service.loadMutedAccounts {
			...
			DispatchQueue.main.async {
				if case let .success(allMutedAccounts) = result {
	                               ...
	                               completion: (() -> Void)? = nil)
	{
		...
		client.run(Login.oauth(clientID: clientApplication.clientID,
			...
			switch loginResult {
			case let .success(loginResult, _):
				login = loginResult
	...
	private func showError<T: UserDescriptionError>(_ error: T, with baseDomain: String) {
		guard case let .pickingDomain(sourceWindow, _) = authorizationState else {
			return
		}
	...
	private func cleanupAuthorizationState() {
		switch authorizationState {
		case let .authorizing(_, sheetWindowController, _, _, _), let .pickingDomain(_, sheetWindowController):
...
extension AuthController: InstancePickerWindowControllerDelegate {
	func authWindow(_: NSWindow, didPickValidBaseDomain baseDomain: String) {
		guard case let .pickingDomain(sourceWindow, _) = authorizationState else { return }
...
extension AuthController // Helpers
{
	...
	enum ValidInstanceCheckErrors: UserDescriptionError {
		...
		var userDescription: String {
			...
			case let .clientError(error): return 🔠("instance.clientError", error.localizedDescription)
			}
		}
	}
}
