...
class AccountsPreferencesController: BaseAccountsPreferencesViewController {
	...
	@IBOutlet private var avatarImageView: FileDropImageView!
	@IBOutlet private var headerImageView: FileDropImageView!

	@IBOutlet private var failurePlaceholderView: NSView!
	@IBOutlet private var failureMessageLabel: NSTextField!
	@IBOutlet private var failureMessageButton: NSButton!
	...
	private var avatarMediaAttachment: MediaAttachment? {
		didSet { selectedAccount?.hasPendingUploads = avatarMediaAttachment != nil || headerMediaAttachment != nil }
	}

	private var headerMediaAttachment: MediaAttachment? {
		didSet { selectedAccount?.hasPendingUploads = avatarMediaAttachment != nil || headerMediaAttachment != nil }
	}
	...
	private var originalAvatarImage: NSImage?
	@objc private(set) dynamic var avatarImage: NSImage?

	private var originalHeaderImage: NSImage?
	@objc private(set) dynamic var headerImage: NSImage?
	...
	private func updateAccountImages(completion: @escaping () -> Void) {
		...
		resourceFetcher.fetchImages(with: [avatarImage, headerImage].compacted()) { results in
			DispatchQueue.main.async {
				if let imageUrl = avatarImage, case let .success(image)? = results[imageUrl] {
				...
				if let imageUrl = headerImage, case let .success(image)? = results[imageUrl] {
	...
	private func prepareToChangeAccount(completion: @escaping (_ canceled: Bool) -> Void) {
		...
		confirmSaveOrDiscardChanges { [weak self] result in
			switch result {
			case .save:
				self?.saveAccount { error in
					guard let error = error else {
						completion(false)
						return
					}
					self?.showSaveErrorAlert(error: error) { tryAgain in
						if tryAgain {
							// Repeat process
	...
	private func saveAccount(completion: @escaping (_ error: Errors?) -> Void) {
		...
		guard let account = accounts?[bounded: tableView.selectedRow], let accountProxy = selectedAccount, let client = Client.create(for: account) else {
			completion(nil)
			return
		}
		let request = Accounts.updateCurrentUser(displayName: accountProxy.displayName, note: accountProxy.note, avatar: avatarMediaAttachment, header: headerMediaAttachment, locked: accountProxy.locked.boolValue, bot: accountProxy.bot.boolValue, fieldsAttributes: accountProxy.fieldsAttributes)
		let futurePromise = Promise<FutureTask>()
		let future = client.run(request, resumeImmediately: true) { [weak self, unowned accountsService, futurePromise] result in
			DispatchQueue.main.async {
				...
				switch result {
				case let .success(updatedAccount, _):
					self?.selectedAccount = AccountBindingProxy(account: updatedAccount, instance: accountProxy.instance, authorizedAccount: account)
	...
	private func showSaveErrorAlert<T: UserDescriptionError>(error: T, completion: @escaping (_ tryAgain: Bool) -> Void) {
		...
		let alert = NSAlert(style: .warning, title: 🔠("preferences.account.failedSave"), message: error.userDescription)
	...
	private func confirmSaveOrDiscardChanges(completion: @escaping (ModalAlertResult) -> Void) {
		guard let window = view.window else { return }
		let alert = NSAlert(style: .warning, title: 🔠("Unsaved changes"), message: 🔠("preferences.account.unsavedChanges"))
	...
	private func handleNewAvatar(imageUrl: URL) {
		createMediaAttachment(with: imageUrl) { mediaAttachment in
	...
	private func createMediaAttachment(with imageUrl: URL, completion: @escaping ((Upload, MediaAttachment)?) -> Void) {
		...
		DispatchQueue.global(qos: .utility).async {
			...
			let attachment = MediaAttachment.other(data, fileExtension: upload.fileExtension, mimeType: upload.mimeType)
			completion((upload, attachment))
		}
	}

	@available(*, deprecated)
	private func set(controlsState _: ControlsState) {
		didChangeControlsState()
	}
	...
	private func setCurrentAccount(_ account: AuthorizedAccount?) {
		...
		let future = client.fetchAccountAndInstance { [weak self] result in
			switch result {
			case let .success((account, instance)):
				DispatchQueue.main.async {
					self?.selectedAccount = AccountBindingProxy(account: account, instance: instance, authorizedAccount: authorizedAccount)
	...
	private func discardChanges() {
		...
		self.selectedAccount = AccountBindingProxy(account: selectedAccount.account, instance: selectedAccount.instance, authorizedAccount: selectedAccount.authorizedAccount)
	...
	func tableView(_ tableView: NSTableView, validateDrop info: NSDraggingInfo, proposedRow row: Int, proposedDropOperation _: NSTableView.DropOperation) -> NSDragOperation {
		guard (info.draggingSource as? NSTableView) == tableView, info.draggingPasteboard.propertyList(forType: .accountTableViewDragAndDropType) is [String: Int] else {
	...
	func tableView(_ tableView: NSTableView, acceptDrop info: NSDraggingInfo, row targetRow: Int, dropOperation _: NSTableView.DropOperation) -> Bool {
		guard let propertyList = info.draggingPasteboard.propertyList(forType: .accountTableViewDragAndDropType), let sourceRow = (propertyList as? [String: Int])?["rowIndex"], let accounts = accounts, (0 ..< accounts.count).contains(sourceRow) else {
			return false
		}
	...
	func tableView(_: NSTableView, shouldSelectRow row: Int) -> Bool {
		guard !hasURLTask, selectedAccount?.hasChanges != true else {
			prepareToChangeAccount { [unowned self] didCancel in
				DispatchQueue.main.async {
					if !didCancel {
...
extension AccountsPreferencesController: AttachmentUploaderDelegate {
	func attachmentUploader(_: AttachmentUploader, finishedUploading _: Upload) {}
	func attachmentUploader(_: AttachmentUploader, updatedProgress _: Double, for _: Upload) {}
	func attachmentUploader(_: AttachmentUploader, produced _: AttachmentUploader.UploadError, for _: Upload) {}
	func attachmentUploader(_: AttachmentUploader, updatedDescription _: String?, for _: Upload) {}
	func attachmentUploader(_: AttachmentUploader, failedUpdatingDescriptionFor _: Upload, previousValue _: String?) {}
}

extension AccountsPreferencesController {
	@IBAction func saveChanges(_ sender: Any?) {
		saveAccount { [unowned self] error in
			guard let error = error else {
				return
			}
			self.showSaveErrorAlert(error: error) { tryAgain in
	...
	@IBAction func discardChanges(_: Any?) {
		discardChanges()
	}
	...
	@IBAction func clickedAvatarWell(_ sender: Any?) {
		...
		if let imageUrl = (sender as? NSDraggingInfo)?.firstDraggedFileURL {
			...
			openPanel.beginSheetModal(for: window) { [unowned self] response in
	...
	@IBAction func retryLoadingAccountInfo(_: Any?) {
		switch controlsState {
		case .couldNotLoadAccount:
