...
class AccountsPreferencesController: BaseAccountsPreferencesViewController {
	...
	private func handleNewAvatar(imageUrl: URL) {
		createMediaAttachment(with: imageUrl) {
			mediaAttachment in
	...
	{
		guard let upload = Upload(fileUrl: imageUrl, imageRestrainer: attachmentUploader.imageRestrainer)
		else {
			completion(nil)
			return
		}
	...
	func tableView(_ tableView: NSTableView, acceptDrop info: NSDraggingInfo,
	               row targetRow: Int, dropOperation _: NSTableView.DropOperation) -> Bool
	{
		guard
